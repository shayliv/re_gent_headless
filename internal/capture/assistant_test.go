...
// runTurn drives one full turn: prompt, a tool call, then Stop.
func runTurn(t *testing.T, recorder *Recorder, meta SessionMetadata, turnID, prompt, toolUseID, finalMessage string) {
	t.Helper()
...
func TestRecorder_CapturesAssistantReasoningFromTranscript(t *testing.T) {
	...
	runTurn(t, recorder, meta, "", "list files", "toolu_1", "Listing the directory.")
...
func TestRecorder_TranscriptCaptureIsIdempotent(t *testing.T) {
	...
	runTurn(t, recorder, meta, "", "first", "toolu_1", "First reply.")
	...
	runTurn(t, recorder, meta, "", "second", "toolu_2", "Second reply.")
...
func TestRecorder_TranscriptCaptureSurvivesCompact(t *testing.T) {
	...
	runTurn(t, recorder, meta, "", "before compact", "toolu_1", "Pre-compact reply.")
	...
	runTurn(t, recorder, meta, "", "after compact", "toolu_2", "Post-compact reply.")
...
func TestRecorder_TranscriptCaptureSurvivesClear(t *testing.T) {
	...
	runTurn(t, recorder, meta, "", "before clear", "toolu_1", "Reply before clear.")

	if err := os.Remove(transcriptPath); err != nil {
		t.Fatalf("remove transcript: %v", err)
	}
	runTurn(t, recorder, meta, "", "after clear", "toolu_2", "Reply after clear.")
...
func TestRecorder_TranscriptCaptureToleratesMissingPath(t *testing.T) {
	...
	runTurn(t, recorder, meta, "turn-1", "do it", "call_1", "done")
