...
// The push transport is exercised in internal/remote (push_test.go) and via
// internal/cli/sync_test.go after the RE-14 server cutover. These tests cover
// the auth gate `rgt push` applies before any network work (RE-9 acceptance
// criteria): push must fail closed when the user is not authenticated.
